test_that("kernel must be a valid kernel",{
  ...
  lambda <- 1
  subdivisions = 10L
  expect_error(cross_validation(not_ker,
                          samples = NULL,
                          H_n,
                          lambda,
                          subdivisions))
...
test_that("samples must be numeric with length greater than 0",{
  ...
  lambda <- 1
  subdivisions = 10L
  expect_error(cross_validation(kernel,
                          samples = c(1, 2, 3, "a"),
                          H_n,
                          lambda,
                          subdivisions))
...
test_that("cross_validation should return a smaller bandwidth fpr bigger samplesize", {
  set.seed(50)
  kernel <- epanechnikov
  ...
  expect_true(bandwidth_50 > bandwidth_500)
})
