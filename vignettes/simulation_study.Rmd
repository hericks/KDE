...
We ran the comparison on 200 repetitions on three densities, three kernels and four different sample sets.\
\
...
In this scenario, the `goldenshluger_lepski` method never really shines, but the `pco_method` performed best at smaller sets of samples.\
...
All in all, there is no surprising result, as the mean integrated squared errors for all of the bandwidth estimators are in a similar range.\

\
Now we will take a closer look if the *kernel* influences the performance of the bandwidth estimators.\
We will not take the mean for the whole set of $ns = \{10, 50, 100, 1000\}$.\
To get a more meaningful comparison of the methods, we will only look at 1000 samples, because we consider them to be a large enough sample size, such that the kernel should not have that much impact on the outcome.
...
Our conclusion above stated, that the `pco_method` performs worst on larger sample sizes. This is true for every kernel.\
The influence of the kernel on the result is not very big, because the mean integrated squared errors are in a very similar range for every kernel. Only the gaussian kernel performed a little bit better than epanechnikov and rectangular.\
The `cross_validation`  worked better on the epanechnikov kernel than `goldenshluger_lepski` and `pco_method`. \ 
Other than that, there seems to be not that much impact of the kernel on the bandwidth estimators at 1000 samples, hence every method gets a similar MISE. 

\
Last but not least, the most important part:\
How do the bandwidth estimators work on different *densities*?\
Like in our experiment for the kernels, the number of samples will be 1000.
...
The remarkably difference of `cross_validation` could motivate a new study, that takes a closer look at the dependency of the performance for our estimatorson on the regularity of the density, that we took samples from.



## Comments On The Study
(Umformulieren, vllt noch anderen stuff in Richtung vergleich zwischen eher glatten funktionen, (nur) stetigen und treppenfunktionen hinzufügen!)\
