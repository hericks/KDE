...
To ensure a clear overview, we list all the parameters that we vary or fix.
...
*kernel*, *kernels*, *k*: fixed on *epanechnikov* kernel for large sample sizes, in the other cases we use a list of *rectangular*, *gaussian*, and *epanechnikov*. \
...
Note: here we would like to apply our observations from the first section and assume that with a large number of samples the choice of the kernel is almost irrelevant. Unfortunately, our computing power is limited, so we run it on $1000$ samples, even if that is not enough to ensure this effect. 
...
Here we set the kernel to *epanencnikov* and use the three densities in *dens_list* that we already used when we studied the influence of the kernel. We select only one kernel and set the number of samples to $1000$, for the same reason like in the section above, but the reader should keep this note in mind. 
TODO:(LINK ZU OBEN ???)
...
To derive relialbe information from this comparison we would need to use much more samples and also compare bigger sets of different densities. Nevertheless we can see a tendency for the optimal value for 

But in some regards we can see, that the default, $\kappa = 1.2$ is perfectly in our range of the optimal kappas we calculated. \ 
For our main comparison, we will use the suggested kappa given in literature.

Now we will try to comprehend the tuning parameter selection for the Penalized Comparison to Overfitting method. \
The suggestion is to set $\lambda = 1$ as default value. \
The parameters will be the same as for the kappa.
...
We can see a similar effect like in our kappa comparison, the smooth kernel can approximate smooth functions a little bit better. \
The mean integrated squared error does not vary as much for a different $\lambda$ as for a varying $\kappa$.
