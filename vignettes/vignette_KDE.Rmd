...
print(dens)
```
Now we can see that the density is an object of the class `Density`. Notice the custom S3 printing method for `Densitiy` objects. In addition to the function, the object is equipped with a support and subdivisions parameter. Both are needed for the `stats::integrate` function to work properly.
...
To see the requirements for constructing a Kernel object and further informations use `?Kernel`.
