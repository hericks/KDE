...
conv <- function(f, g, x_min, x_max, N = 300) {
  ...
  x_out <-
    seq(2 * x_min + s, 2 * x_max - s, len = 2 * N - 1) # offset s is artifact of discretization
  list(x = x_out, y = y_out)
}

double_kernel_estim <-
  function(kernel, samples, h, h_ap, grid_size) {
    n <- length(samples)
    ...
    convolution <-
      conv(kernel_h_ap$fun, kernel_h$fun, x_min, x_max, grid_size)
...
bias_estim <-
  function(kernel,
    ...
    for (h_ap in H_n) {
      ...
      if (f_double$support[2] < kde_h_ap$support[1] |
          kde_h_ap$support[2] < f_double$support[1]) {
        z1 <-
          integrate(function(x) {
            f_double(x) ^ 2
          }, f_double$support[1], f_double$support[2])
        z2 <-
          integrate(function(x) {
            kde_h_ap(x) ^ 2
          }, kde_h_ap$support[1], kde_h_ap$support[2])
        l2 <- z1[[1]] + z2[[1]]
      }
      else{
        l2 <- integrate(
          function(x) {
            (f_double$fun(x) - kde_h_ap$fun(x)) ^ 2
          },
...
goldenshluger_lepski_method_2 <-
  function(kernel,
    ...
    for (h in H_n) {
      print("-------------------------")
      res_temp <-
        bias_estim(kernel, samples, h, H_n, kappa, var_est, grid_size) + 2 * kappa * var_est(h)
      print(res_temp)
      res <- c(res, res_temp)
    }
...
find_support <- function(fun) {
  ...
  non_zero_indices <-
    which(sapply(testing_values, function(x)
  ...
  lower_bound <-
    ifelse(lower_index < 1,-Inf, testing_points[lower_index])
  upper_bound <-
    ifelse(upper_index > length(testing_points), Inf, testing_points[upper_index])
